...
| **調査モード** | Cursor + Browser Automation（ネットワーク利用必須）、検索キーワード例: "Next.js 15.4 release notes", "Next.js 15.5.3 GitHub" |
| **重要度** | 🟡 中 |
| **重要度根拠** | Breaking要素は限定的だが、主要ビルドフローや動的ルーティングへの影響があり、早期に把握する必要があるため |
| **調査者** | - |

> **重要度レベル定義（サンプル）**  
> - 🔴 **高**: セキュリティ脆弱性対応や互換性破壊で本番が停止するリスクが高い変更  
> - 🟡 **中**: 主要機能の安定性や品質に影響し、放置すると障害リスクが高まる変更  
> - 🟢 **低**: 任意導入の改善や小規模追加で、直ちに障害へ直結しない変更  
> 重要度を記載する際は、必ず上記のどれに該当するかの根拠を併記してください。
...
- **重要度 / 根拠**: 🟡 CI/CDの所要時間やDXへ大きく影響し投資対効果が高いが、既存ビルドを即座に破壊するリスクは低いため
...
- **重要度 / 根拠**: 🔴 WindowsベースのCI/開発環境ではビルド失敗でデプロイが停止するため
...
- **重要度 / 根拠**: 🔴 動的ルートが本番ユーザー操作に直結し、レイアウト崩壊で致命的UX劣化を招くため
...
#### スコープホイスト変数処理（15.4.4）
- **重要度 / 根拠**: 🟡 一部コンポーネントで実行時エラーが発生し得るが影響範囲は限定的
...
#### 非同期API化（15.0〜）
- **重要度 / 根拠**: 🔴 同期アクセスのままだとランタイムエラーでページが落ちるため即時対応必須
- **対象API**: 
  - `headers()`
  - `cookies()`
  - `params`
